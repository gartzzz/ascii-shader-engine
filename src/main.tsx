...
function App() {
  ...
  const buildEmbeddedVanillaHtml = (dataUrl: string) => `<!doctype html>\n<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ASCII Shader — plug & play</title><style>html,body{margin:0;height:100%;background:#111}#hero{width:100%;min-height:60vh;aspect-ratio:16/9;height:100vh;position:relative;overflow:hidden}</style></head><body>\n<div id="hero" style="width:100%;min-height:60vh;aspect-ratio:16/9"></div>\n<script src="https://ascii-shader-engine.vercel.app/ascii-shader.umd.js"><\/script>\n<script>\nAsciiShader.createAsciiShader({ target: "#hero", src: ${JSON.stringify(dataUrl)}, preset: ${JSON.stringify(preset)}, charsetPreset: ${JSON.stringify(charsetPreset)}, colorMode: ${JSON.stringify(colorMode)}, foregroundColor: ${JSON.stringify(foregroundColor)}, backgroundColor: ${JSON.stringify(backgroundColor)}, duotoneDark: ${JSON.stringify(duotoneDark)}, duotoneLight: ${JSON.stringify(duotoneLight)}, cellSize: ${cellSize}, interaction: ${JSON.stringify(interaction)} });\n<\/script>\n</body></html>`;
  ...
  return (
    <main className={`demo-shell format-${format}`}>
      ...
      <aside className="control-panel">
        ...
        <div className="panel-actions"><button type="button" onClick={() => setDebug((value) => !value)}>Debug {debug ? "on" : "off"}</button><button type="button" className="primary" onClick={exportPlugAndPlay}>Copy Plug & Play HTML</button></div>
        <div className="panel-actions"><button type="button" className="primary" onClick={exportStandalone}>Download Plug & Play HTML</button></div>
